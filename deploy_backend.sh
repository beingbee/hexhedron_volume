...
sam build  || { echo "Error!! build failure"; exit 1; }
sam deploy --guided --stack-name $STACK_NAME  || { echo "Error!! deploy failure"; exit 1; }
...
export API_BASE_URL=`aws cloudformation describe-stacks --stack-name $STACK_NAME --query "Stacks[0].Outputs[?OutputKey=='HexVolFunctionApi'].OutputValue" --output text`
